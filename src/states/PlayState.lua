...
function PlayState:update(dt)
    ...
    function swap(highlightedTile,newTile,board)  
        -- swap grid positions of tiles
        local tempX = highlightedTile.gridX
        local tempY = highlightedTile.gridY

        local tempXnew = newTile.gridX
        local tempYnew = newTile.gridY
        ...
        board.tiles[highlightedTile.gridY][highlightedTile.gridX] =
            highlightedTile
    ...
    function swapMatches(highlightedTile,newTile,board)
        local tempX = highlightedTile.gridX
        local tempY = highlightedTile.gridY
        ...
        if not self.board.matchBool then 
            return false
        else 
            return true
        end
    ...
    function emptyBoard(board)
        for y = 1,8 do 
            for x = 1,8 do
                local highlightedTile = board.tiles[y][x]

                if not y == 8 then
                    local newTile = board.tiles[y+1][x]
                    if swapMatches(highlightedTile,newTile,board) then
                        return false
                    end
                end
                ...
                if not x == 8 then
                    local newTile = board.tiles[y][x+1]
                    if swapMatches(highlightedTile,newTile,board) then
                        return false
                    end
                end
...
--[[
    if not emptyBoard(self.board) then
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                local highlight = self.highlightedTile 
                local swapMatches = swapMatches(highlight,newTile,self.board)


                if swapMatches then 
                    self.board = swap(highlight,newTile,self.board)
                else
                    gSounds['error']:play()
                end                
            end
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        self.score = self.score + self.board.tilesRemoved*50
        self.board.tilesRemoved = 0
